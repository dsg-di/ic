...
import gr.uoa.di.dsg.ic.experiments.ICBaseSlaveSerial;
...
public class ICMainExecutorSerial extends ICBaseSlaveSerial {
	
	private static ICMainExecutorSerial icMainExe = null;
	...
	public ICMainExecutorSerial(int nodeId) {
		this.nodeId = nodeId;
...
//		}
	...
	public void run() {
		...
		if(inWarmupPhase && warmupCounter == 1)
			exchangeHelloMessages();
		
		int appID;
		if(inWarmupPhase)
			appID = warmupCounter - 1;
		else
			appID = warmupCounter + instanceCounter - 1;
		
		Application app = applicationGetter.getApp(String.valueOf(appID));

		// Avoid the rendezvous in the first warm-up round.
		if(!inWarmupPhase || warmupCounter != 1) {
			try {
		...
		if(!inWarmupPhase)
			markStartTime(instanceCounter);
		app.start(this.nodeId, Integer.toString(appID * this.N + this.nodeId));
	}
	
	public void processResult(String applicationID, List<String> res) {
		if(!inWarmupPhase)
			markDataPoint(instanceCounter);
		...
		for(int i = 0; i < N; ++i) {
			...
			String expectedResult = String.valueOf(((N * Integer.valueOf(applicationID)) + i));
		...
		// Notify the ICMaster process that I have terminated my current execution.
		comm.send(nodes[N], new ExperimentCompleteMessage());

		// Check if we have reached the final execution.
		if(!inWarmupPhase && instanceCounter == K) {
			// Terminate the monitoring thread.
			if (monitoringRunnable != null)
				monitoringRunnable.terminate();

			// Create the output file that will contain all execution times.
			appendTimeToFile();
			...
			if (monitoringRunnable != null) {
				...
				try {
					...
					monitoringRunnable.print(new String(outputDirectory + "/" + N + "." + K + "." + nodeId + "." + algorithm + ".serial"));
				} catch (InterruptedException ex) {
	...
	public void processFirstPhaseCompletion(String applicationID) {
		if(!inWarmupPhase)
			markFirstPhaseEndTime(instanceCounter);
	}
	...
	private void processExperimentStartMessage(ExperimentStartMessage experimentStartMsg, Node source) {
		...
		if(warmupCounter < warmupRounds) {
			...
			if(instanceCounter < K) {
				instanceCounter += operationsInEachExecution;
				run();
			}
		}
	}
	...
	private void processExperimentTerminationMessage(ExperimentTerminationMessage experimentTerminationMsg, Node source) {
		...
		if (threadMXBean.isThreadCpuTimeSupported()) {
			double CPUTime = threadMXBean.getThreadCpuTime(Thread.currentThread().getId()) / (double) (1000 * 1000);
			System.out.printf("[Node %d]: Total CPU time (in ms): %.4f\n", nodeId, CPUTime);
		}
	...
	public static void main(String[] args) {
		...
		icMainExe = new ICMainExecutorSerial(nodeID);
		...
		icMainExe.executionMode = "serial";
