...
public class TestConsistentBroadcastWithDigitalSignatures {
	
	private int numNodes = 4;
	private AbstractTestNode[] allNodes = new AbstractTestNode[numNodes];
	private int TOTAL_EXECUTIONS = 1;
	...
	private class TestNode extends AbstractTestNode implements Application {
		...
		public TestNode(int nodeId, int port) {
			...
			this.broadcastModule = new ConsistentBroadcast();
		}
		...
		public void process(Message msg) {
			...
			if((++counter) == TOTAL_EXECUTIONS * TestConsistentBroadcastWithDigitalSignatures.this.numNodes)
