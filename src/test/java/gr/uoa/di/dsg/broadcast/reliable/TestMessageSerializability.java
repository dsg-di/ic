...
public class TestMessageSerializability extends TestCase {
    ...
    public void testApp() throws NoSuchAlgorithmException, InvalidKeyException, SignatureException, IOException {
    	...
    	RBSSendMessage sendMessage = new RBSSendMessage(IC_ID, CONSENSUS_ID, PROCESS_ID, BROADCAST_ID, sampleValue);
    	RBSSendMessage copyOfSendMessage = RBSSendMessage.deserialize(sendMessage.serialize());
    	System.out.println(sendMessage.serialize().length);
    	
    	RBSEchoMessage echoMessage = new RBSEchoMessage(IC_ID, CONSENSUS_ID, PROCESS_ID, BROADCAST_ID, sampleValue);
    	echoMessage.setSignature(nodeIDs, signatures);
    	RBSEchoMessage copyOfEchoMessage = RBSEchoMessage.deserialize(echoMessage.serialize());
